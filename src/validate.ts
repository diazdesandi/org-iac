...
// Structural duplicates (repo names, team slugs, etc.) live in Zod schemas.
// Only validations that need to see multiple domains belong here.
...
function validateTeamRefs(config: InfraConfig): ValidationIssue[] {
  ...
  for (const [repoName, access] of Object.entries(teams.repo_access)) {
    if (!repoNames.has(repoName)) {
      issues.push(
        issue(`teams.repo_access.${repoName}`, `unknown repo "${repoName}"`),
      );
    }
...
function validateRulesetPatterns(config: InfraConfig): ValidationIssue[] {
  ...
  for (const ruleset of rulesets) {
    ...
    for (const raw of ruleset.conditions.include) {
